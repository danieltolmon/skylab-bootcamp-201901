...
describe('logic', () => {
    ...
    describe('retrieve artist', () => {
        let comment = {
            artistId: '6tbjWDEIzxoDsBA1FuhfPW', // madonna
            text: 'que artista mas chulo',
            date: new Date
        }
        ...
        it('should succeed on matching query', () => {
            ...
            return logic.retrieveArtist(comment.artistId, _id)
                .then(({ id, name, comments }) => {
                    expect(id).toBe(comment.artistId)
                    expect(name).toBe('Madonna')
                    expect(comments[0].text).toBe('que artista mas chulo')
                    expect(comments[0].artistId).toBe('6tbjWDEIzxoDsBA1FuhfPW')
                    expect(comments[0].userId).toBe(_id)
                })
        })
