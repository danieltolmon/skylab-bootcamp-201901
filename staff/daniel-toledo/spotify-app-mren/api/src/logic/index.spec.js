...
const { env: { TEST_DB_URL, SPOTIFY_API_TOKEN, SECRET_JSON } } = process
...
describe('logic', () => {
        ...
        it('should succeed on correct data', () =>
            logic.toggleFavoriteArtist(_id, _token, artistId)
                ...
                .then(user => {
                    ...
                    expect(user.favoriteArtist).toBeDefined()
                    expect(user.favoriteArtist.length).toBe(1)
                    expect(user.favoriteArtist[0]).toBe(artistId)
                ...
                .then(user => {
                    ...
                    expect(user.favoriteArtist).toBeDefined()
                    expect(user.favoriteArtist.length).toBe(0)
                })
        )
    ...
    false && describe('add comment to artist', () => {
        const name = 'Manuel'
        const surname = 'Barzi'
        const targetId = '6tbjWDEIzxoDsBA1FuhfPW' // madonna
        const text = 'amazing comment'
        let password, email, userId, commentId
        ...
        it('should succeed on correct data', () => {
            logic.addComment(userId, 'artist', targetId, text)
                .then(id => {
                    expect(id).toBeDefined()
                    commentId = id
                    Comment.findById(id).lean()
                        .then(_comment => {
                            expect(_comment._id.toString()).toBe(commentId)
                            expect(_comment.user.toString()).toBe(userId)
                            expect(_comment.targetId).toBe(targetId)
    ...
    describe('list comments from artist', () => {
        ...
        beforeEach(async () => {
            ...
            return User.create({ name, surname, email, password: hash })
                .then(({ id }) => userId = id)
                .then(() => Comment.create({ user: userId, target: 'artist', targetId, text }))
                .then(() => Comment.create({ user: userId, target: 'artist', targetId, text: text2 }))
                .then(() => Comment.create({ user: userId, target: 'artist', targetId, text: text3 }))

        })

        it('should succeed on correct data', () =>
            logic.listComments('artist', targetId)
                .then(comments => {
                    expect(comments).toBeDefined()
                    expect(comments.length).toBe(3)

                    comments.forEach(({ id, user, targetId, target, date }) => {
                        expect(id).toBeDefined()
                        expect(user.toString()).toEqual(userId)
    ...
    //                 expect(tracks.constructor === Array).toBeTruthy
    //             })
    //     })
    ...
    //         expect(() => logic.retrieveTracks(albumId)).toThrowError(`${albumId} is not a string`)
    //     })
    ...
    //     it('should succeed on correct data', () => {
    //         return logic.toggleFavoriteAlbum(_id, albumId)
    //             .then(() => users.findByUserId(_id))
    //             .then(user => {
    //                 expect(user.id.toString()).toBe(_id)
    //                 expect(user.name).toBe(name)
    //                 expect(user.surname).toBe(surname)
    //                 expect(user.email).toBe(email)
    ...
    //     it('should succeed on correct data', () =>
    //         logic.toggleFavoriteTrack(_id, trackId)
    //             .then(() => users.findByUserId(_id))
    //             .then(user => {
    //                 expect(user.id.toString()).toBe(_id)
    //                 expect(user.name).toBe(name)
    ...
    //                 return logic.toggleFavoriteTrack(_id, trackId)
    //             })
    //             .then(() => users.findByUserId(_id))
    //             .then(user => {
    //                 expect(user.id.toString()).toBe(_id)
